...
void EEstudies_orange_mustd_daily(){
	...
	TString outputdir = "Plots/20121119/EEstudies/continued/orange/daily/mustd/";//normalized";
	...
	for(int nn = 0; nn<2; ++ nn){//producer
		...
		g_slopes[n][nn]->GetYaxis()->SetTitle("slope (m)"); g_slopes[n][nn]->GetXaxis()->SetTitle("time");
		g_slopes[n][nn]->SetMarkerSize(1); g_slopes[n][nn]->SetMarkerStyle(20);

		hname = (string)"intercepts_vs_time_Eta_" + string_feta[n] + string_prod[nn];
		g_onevalues[n][nn] = new TGraphErrors();
		g_onevalues[n][nn]->SetNameTitle(hname.c_str(), hname.c_str());
		g_onevalues[n][nn]->GetYaxis()->SetTitle("intercept"); g_slopes[n][nn]->GetXaxis()->SetTitle("time");
...
bool badsthsingle = ( (k==0&&((i==9&&j==33)||(i==47&&j==78)))||(k==1&&( (i==39&&j==46)||(i==52&&j==4)||(i==78&&j==76)||(i==83&&j==21)||(i==84&&j==24)||(i==86&&j==24)||(i==94&&j==64)||(i==19&&j==84)||(i==24&&j==77) ) )||(k==1&&(i==91||i==92)&&(j>=21&&j<=25) )||bad2012||Onoisecrystals || Onoisecrystals2||Onoisecrystals3||Onoisecrystalsp5||Ospecialcrystals||risingcrystals||partiallyrising||partiallyrising2 );
	...
	cout << "now start the fitting of mustd vs VPTdiff --> slopes onevalues " << endl;
	int oneday = 86400;
	...
	for(int nn = 0; nn<2; ++ nn){//producer
		...
		a_mustd_vs_VPTPNdiff[n][nn]->GetYaxis()->SetTitle("VPT/PN_{max}-VPT/PN_{min}"); a_mustd_vs_VPTPNdiff[n][nn]->GetXaxis()->SetTitle("#mu_{std}");
	...
	for(int nn = 0; nn<2; ++ nn){//producer
		...
		if(printtime){
			...
			haxis->GetXaxis()->SetRangeUser(0.0,2.0);
			haxis->GetXaxis()->SetLimits(0.0,2.0);
	...
	*/
	...
	col->cd();
	Legend1->SetHeader("russian crystals");
	col->SetName( "intercepts_vs_time_daily_russian_burnin" );
	col->SetTitle("intercepts_vs_time_daily_russian_burnin" );
	haxis->Draw("hist");
	...
	col->cd();
	Legend1->SetHeader("chinese crystals");
	col->SetName( "intercepts_vs_time_daily_chinese_burnin" );
	col->SetTitle("intercepts_vs_time_daily_chinese_burnin" );
	haxis->Draw("hist");
	...
	if(savefinalplots){
	TFile *newfile = new TFile ("FileOutputs/20121130_mustd_vs_VPTPNdiff_orange_fit_slopes_intercepts_daily.root", "RECREATE");
	newfile->cd();
	for(int n = 0; n<8; ++n){//eta bin
	for(int nn = 0; nn<2; ++ nn){//producer
		g_slopes[n][nn]->Write();
