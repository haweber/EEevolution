...
*/
	...
	TGraph *lumi_vs_time = new TGraph();
	lumi_vs_time->SetTitle("lumi_vs_time"); lumi_vs_time->SetName("lumi_vs_time"); lumi_vs_time->SetMarkerColor(kRed); lumi_vs_time->SetMarkerStyle(20);
...
//			if(apdpnoled>0) g_vptpn_oled[iX][iY][k] -> SetPoint(g_vptpn_oled[iX][iY][k]->GetN(), toled, apdpnoled);
		...
		//	if(fwhm>0)      g_EE_fwhm[iX][iY][k]    -> SetPoint(g_EE_fwhm[iX][iY][k]->GetN(),    tlas , fwhm);
			...
			//EB
			/*else{
			int iXX = iX+85;//as ix starts at -85
			if(apdpnlas>0) g_apdpn_las[iXX][iY] -> SetPoint(g_apdpn_las[iXX][iY]->GetN(), tlas , apdpnlas);
			if(apdpnled>0) g_apdpn_led[iXX][iY] -> SetPoint(g_apdpn_led[iXX][iY]->GetN(), tled , apdpnled);
			if(apdpnrat>0) g_apdpn_rat[iXX][iY] -> SetPoint(g_apdpn_rat[iXX][iY]->GetN(), tlas , apdpnrat);
			if(ampl>0) g_EB_ampl[iXX][iY] -> SetPoint(g_EB_ampl[iXX][iY]->GetN(), tlas , ampl);
			if(fwhm>0) g_EB_fwhm[iXX][iY] -> SetPoint(g_EB_fwhm[iXX][iY]->GetN(), tlas , fwhm);
			}*/
		}

		float lumilas = x.lumi[0];
		float lumiled = x.lumi[1];
		float lumioled= x.lumi[2];
		if(tlas>1200000000 && lumilas>0) lumi_vs_time->SetPoint(lumi_vs_time->GetN(), tlas, lumilas);
//		if(tled>1200000000 && tled!=tlas && lumiled>0) lumi_vs_time->SetPoint(lumi_vs_time->GetN(), tled, lumiled);
//		if(toled>1200000000 && toled!=tled && toled!=tlas && lumioled>0) lumi_vs_time->SetPoint(lumi_vs_time->GetN(), toled, lumioled);
...
//	TFile *newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20121102_EE_VPT_over_PN_WG1_period32_to_period52.root", "RECREATE");
	TFile *newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20121102_Lumi_vs_time.root", "RECREATE");
	newFile->cd();
//	lumi_vs_time->Sort();
	lumi_vs_time->Write();
//	for (int k = 0; k < 2; ++k){
...
//	files.push_back(TFile::Open("/shome/haweber/ECAL/DataLaser/small_ntu_data_00173378-00176049.root"));
