...
void EEstudies_continued_v2(){
	...
	char gname[101];
	TString outputdir = "/shome/haweber/ECAL/Laser/Analysis/Plots/20120224/EEstudies/continued/v2c/notnormalized";//normalized";//normalized if normalize VPT/PN
	if(normalized) outputdir = "/shome/haweber/ECAL/Laser/Analysis/Plots/20120224/EEstudies/continued/v2c/normalized/withSashaUnScaled";
//	Util::MakeOutputDir(outputdir);
	TString outputdirac = "/shome/haweber/ECAL/Laser/Analysis/Plots/20120224/EEstudies/continued/v2c/normalized/allcrystalswithmustd";//normalized if normalize VPT/PN
	if(normalized) outputdirac = "/shome/haweber/ECAL/Laser/Analysis/Plots/20120224/EEstudies/continued/v2c/notnormalized/allcrystalswithmustd";
...
//	TFile *vpt_values = TFile::Open("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120403_EE_VPT_over_PN_new2sorted.root");
	...
	//		if(fabs(eta)<2.8) continue;
		...
		for(int n = 0; n<g_vptpn_las[i][j][k]->GetN(); ++n){
			...
			if(y>1.1 && normalized) continue;//veto outliers
			if(y<0.565 && normalized) continue;
			//kill crazy crystals
...
*/
	...
	//	TF1 *scalefuncEB = new TF1("scalefuncEB", "(1./1.)*sqrt(1.*exp(-4.11065 + 0.258478*x - 0.*x*x))", 0., 1.6);//tried forfactor 1/20
		...
		//factor 3.464398 = ECAL TDR Fig.2.7 / sasha's function
...
//		else           haxis->GetYaxis()->SetRangeUser(0.,0.22);
	...
	TFile *newFile;
	if(normalized) newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_savecopy_TESTEWRIAC_withSasha_unscaled.root", "RECREATE");//0312->0410; //muSIC <-> mustd
