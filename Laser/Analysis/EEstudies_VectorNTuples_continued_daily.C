...
void EEstudies_VectorNTuples_continued_daily(){
	...
	TString outputdir = "Plots/20121119/EEstudies/continued/daily";//normalized";
	...
	for(int nn = 0; nn<2; ++ nn){//producer
		...
		g_slopes[n][nn]->GetYaxis()->SetTitle("#mu_{std} slope (m)"); g_slopes[n][nn]->GetXaxis()->SetTitle("time");
		g_slopes[n][nn]->SetMarkerSize(1); g_slopes[n][nn]->SetMarkerStyle(20);

		hname = (string)"intercept_vs_time_Eta_" + string_feta[n] + string_prod[nn];
		g_intercepts[n][nn] = new TGraphErrors();
		g_intercepts[n][nn]->SetNameTitle(hname.c_str(), hname.c_str());
		g_intercepts[n][nn]->GetYaxis()->SetTitle("#mu_{std} intercept"); g_slopes[n][nn]->GetXaxis()->SetTitle("time");
		g_intercepts[n][nn]->SetMarkerSize(1); g_intercepts[n][nn]->SetMarkerStyle(20);

		//create later
		//hname = (string)"fit_mustd_vs_VPTPNdiff_Eta_" + string_feta[n] + string_prod[nn];
		//afit_mustd_vs_VPTPNdiff[n][nn] = new TF1(aname.c_str(), "[0]+[1]*(x-0.0)", 0.0, 2.0);
		...
		g_intercepts[n][nn] ->SetLineColor(color);   g_intercepts[n][nn] ->SetMarkerColor(color);
	...
	cout << "now start the fitting of mustd vs VPTdiff --> slopes intercepts " << endl;
	int oneday = 86400;
	...
	//		if(fabs(eta)<2.8) continue;
			...
			russchin << mustd;
			rs = russchin.str();
			if(prod==1) seta = (string)"_eta_" + es + (string)"_russian_mustd_" + rs;
			if(prod==2) seta = (string)"_eta_" + es + (string)"_chinese_mustd_" + rs;
		}
		else{
	...
	for(int nn = 0; nn<2; ++ nn){//producer
		...
		*/
		...
		int incepbin = g_intercepts[n][nn]->GetN();
		//check
		...
		g_intercepts[n][nn]->SetPoint(     incepbin, d, incep   );
		g_intercepts[n][nn]->SetPointError(incepbin, 0, inceperr);
		//if((d-1293836400+1293840000)%(25*86400)==0) cout << "d " << d << " has slope " << slope << "+/-" << slopeerr<< " intercept " << incep<<"+/-"<<inceperr<< " at etaind/prodind " << n << "/" << nn << " check " << g_slopes[n][nn]->GetN() << " " << g_intercepts[n][nn]->GetN() << " " << endl;
	...
	for(int nn = 0; nn<2; ++ nn){//producer
		...
		for(int nnn=0; nnn<g_intercepts[n][nn]->GetN();++nnn){
			double dx,dy;
			g_intercepts[n][nn]->GetPoint(nnn,dx,dy);
			if(dy>incepmax) incepmax = dy;
			if(dy<incepmin) incepmin = dy;
			if(dx>timemax ) timemax  = dx;
			if(dx<timemin ) timemin  = dx;
			//cout << "incep " << n << " " << nn << " point " << nnn << " has time " << (int)dx << " and intercept " << dy << endl;
		}
	}}
	/*
	...
	*/
	...
	haxis->GetYaxis()->SetTitle("#mu_{std} slope (m)"); 
	TCanvas *col = new TCanvas("col", "col", 60, 0, 1575, 1180);
	col->cd();
	col->Clear();
	...
	haxis->SetNameTitle("intercepts_vs_time", "intercepts vs. time");
	haxis->SetMinimum(incepmin);
	haxis->SetMaximum(incepmax);
	haxis->GetYaxis()->SetTitle("#mu_{std} intercept"); 
	...
	haxis->Draw("hist");
	for(int n = 0; n<8; ++n) {if(g_intercepts[n][0]->GetN()>0) g_intercepts[n][0]->Draw("P");}
	Legend1->Draw("same");
	col->Update();
	outname = col->GetName();
	Util::PrintNoEPS(col, outname, outputdir);
	...
	haxis->Draw("hist");
	for(int n = 0; n<8; ++n) {if(g_intercepts[n][1]->GetN()>0) g_intercepts[n][1]->Draw("P");}
	Legend1->Draw("same");
	col->Update();
	outname = col->GetName();
	Util::PrintNoEPS(col, outname, outputdir);
	...
	if(savefinalplots){
	TFile *newfile = new TFile ("FileOutputs/20121120_mustd_vs_VPTPNdiff_VectorNTuples_fit_slopes_intercepts_daily.root", "RECREATE");
	newfile->cd();
	for(int n = 0; n<8; ++n){//eta bin
	for(int nn = 0; nn<2; ++ nn){//producer
		g_slopes[n][nn]->Write();
		g_intercepts[n][nn]->Write();
	}}
	newfile->Close();
