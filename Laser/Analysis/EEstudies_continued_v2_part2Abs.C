...
void EEstudies_continued_v2_part2Abs(){
	...
	TGraph* h_mustd_vs_VPTPNdiff[8][2];
	TGraph* h_mustd_vs_VPTPNdiff_scaled[8][2];
	TH1D* h_mustd_vs_VPTPNdiff_distancefit[8][2];
	TH1D* h_mustd_vs_VPTPNdiff_scaled_distancefit[8][2];
	TH1D* h_intercept_vs_eta[1][2];
	TH1D* h_intercept_vs_eta_scaled[1][2];
	TH1D* h_slope_vs_eta[1][2];
	TH1D* h_slope_vs_eta_scaled[1][2];
	TH1D* h_RMS_intercept[1][2];//Put RMS and Av in title here
	TH1D* h_RMS_intercept_scaled[1][2];
	TH1D* h_RMS_slope[1][2];
	TH1D* h_RMS_slope_scaled[1][2];
	vector<double> removedX[8][2];
	vector<double> removedY[8][2];
	vector<double> removedscX[8][2];
	vector<double> removedscY[8][2];
	for(int n=0;n<8;++n){
	for(int m=0;m<2; ++m){
		removedX[n][m].clear();
		removedY[n][m].clear();
	...
	string string_eta[8] = {"1p4", "1p6", "1p8", "2p0", "2p2", "2p4", "2p6", "2p8"};
	string string_prod[2] = {"__russ", "__chin"};
	string string_cap[1] = {"_"};
	TFile *oldFile;
...
//	if(normalized) oldFile = TFile::Open("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_savecopy_TESTEWRIAC_withTDR.root");//mustd <->muSIC
//	if(normalized) oldFile = TFile::Open("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_savecopy_TESTEWRIAC_withSasha_unscaled.root");//mustd <->muSIC
	if(normalized) oldFile = TFile::Open("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_savecopy_TESTEWRIAC_withSasha_scaled.root");//mustd <->muSIC
	...
	TString outputdir;
	if(normalized) outputdir = "/shome/haweber/ECAL/Laser/Analysis/Plots/20120224/EEstudies/continued/v2c/normalized/withSashaScaled/fitted";//normalized if normalize VPT/PN // v2b/normalized <-> v2b/SIC/normalized [--> v2c/normalized/fitted THESE ARE WRONG RIGHT NOW!!!!!!!!!!!!!!]
	//normalized/fitted <--> normalized/withSashaUnScaled/fitted <--> normalized/withSashaScaled/fitted
	// v2x/normalized <-> v2x/SIC/normalized
	else outputdir = "/shome/haweber/ECAL/Laser/Analysis/Plots/20120224/EEstudies/continued/v1119/notnormalized/fitted";
	Util::MakeOutputDir(outputdir);
//	outputdir = "/shome/haweber/ECAL/Laser/Analysis/Plots/forFrancesca";//forFrancesca
	for(int n = 0; n<8; ++n){
	for(int nn = 0; nn<2; ++ nn){
		string hname = (string)"mustd__vs__VPTPNmax-VPTPNmin__AbsEta_" + string_eta[n] + string_prod[nn];
		string hname_s = (string)"mustd__vs__VPTPNmax-VPTPNmin__AbsEta_" + string_eta[n] + string_prod[nn] + (string)"__scaled";
		...
		for(int m = h_mustd_vs_VPTPNdiff[n][nn]->GetN()-1; m>=0; --m){
			...
			if(y>0.4  || (n==0&&nn==0&&y>0.1&&x>0.2&&x<0.8)){
		...
		hname = (string)"distancefit__AbsEta__" + hname;
		hname_s = (string)"distancefit__AbsEta__" + hname_s;
	...
	for(int n = 0; n<1; ++n){
	for(int nn = 0; nn<2; ++ nn){
		string hname = (string)"intercept_vs_AbsEta" + string_cap[n] + string_prod[nn];
		string hname_s = (string)"intercept_vs_AbsEta__scaled" + string_cap[n] + string_prod[nn];
		double el, eu; el=1.4;eu=3.0;
		...
		hname = (string)"slope_vs_AbsEta" + string_cap[n] + string_prod[nn];
		hname_s = (string)"slope_vs_AbsEta__scaled" + string_cap[n] + string_prod[nn];
...
//			h_slope_vs_eta_scaled[n][nn]->SetMinimum(0.); h_slope_vs_eta_scaled[n][nn]->SetMaximum(0.115);
		...
		hname = (string)"intercept_RMS_Abs" + string_cap[n] + string_prod[nn];
		hname_s = (string)"intercept_RMS_Abs__scaled" + string_cap[n] + string_prod[nn];
		...
		hname = (string)"slope_RMS_Abs" + string_cap[n] + string_prod[nn];
		hname_s = (string)"slope_RMS_Abs__scaled" + string_cap[n] + string_prod[nn];
	...
	TF1 *fit_mustd_vs_VPTPNdiff_scaled[8][2];
	TF1 *fit_mustd_vs_VPTPNdiff[8][2];
	for(int n = 0; n<8; ++n){
	for(int nn = 0; nn<2; ++ nn){
		string hname = (string)"fit__mustd__vs__VPTPNmax-VPTPNmin__AbsEta_" + string_eta[n] + string_prod[nn];
		string hname_s = (string)"fit__mustd__vs__VPTPNmax-VPTPNmin__AbsEta_" + string_eta[n] + string_prod[nn] + (string)"__scaled";
		//if(hname==(string)"mustd__vs__VPTPNmax-VPTPNmin__eta_m1p6__chin")
		...
		/*if(normalized){
			if(n>5)            fit_mustd_vs_VPTPNdiff[n][nn]->SetParameter(0, 0.07);//eta >2.6: x0=0.0 ->0.07; x0=0.2 ->0.05
			else if(n>3)       fit_mustd_vs_VPTPNdiff[n][nn]->SetParameter(0, 0.04);//eta >2.2: x0=0.0 ->0.04; x0=0.2 ->0.03
			else               fit_mustd_vs_VPTPNdiff[n][nn]->SetParameter(0, 0.01);//eta >1.4: x0=0.0 ->0.01; x0=0.2 ->0.01
			if(n>5)            fit_mustd_vs_VPTPNdiff[n][nn]->SetParameter(1, 0.09);
			else if(n>3)       fit_mustd_vs_VPTPNdiff[n][nn]->SetParameter(1, 0.05);
		...
		//	else               fit_mustd_vs_VPTPNdiff_scaled[n][nn]->SetParameter(0, 0.02);//eta>1.4: x0=0.0 ->; for x0=0.2 -> 0.01
			if(n>2)            fit_mustd_vs_VPTPNdiff_scaled[n][nn]->SetParameter(0, 0.5);//eta>1.8: x0=0.0 ->; for x0=0.2 -> 
			else               fit_mustd_vs_VPTPNdiff_scaled[n][nn]->SetParameter(0, 0.7);//eta>1.4: x0=0.0 ->; for x0=0.2 -> 
			if(n>2)            fit_mustd_vs_VPTPNdiff_scaled[n][nn]->SetParameter(1, 0.07);
			else               fit_mustd_vs_VPTPNdiff_scaled[n][nn]->SetParameter(1, 0.5);
		}
		else{
...
/*//forFrancesca*/		haxis->SetNameTitle(h_mustd_vs_VPTPNdiff[n][nn]->GetName(), h_mustd_vs_VPTPNdiff[n][nn]->GetTitle());
...
/*		haxis->SetName("haxis");
		haxis->SetTitle("");
		haxis->GetXaxis()->SetTitle("#mu_{std} (m^{-1})");
		haxis->GetYaxis()->SetTitle("response charge [a.u.]");
		haxis->SetXTitle("#mu_{std} (m^{-1})");
		haxis->SetYTitle("response charge [a.u.]");
*/		haxis->Draw();
...
/*//forFrancesca*/		haxis->SetNameTitle(h_mustd_vs_VPTPNdiff_scaled[n][nn]->GetName(), h_mustd_vs_VPTPNdiff_scaled[n][nn]->GetTitle());
		haxis->GetXaxis()->SetRangeUser(0.,2.);
		haxis->GetXaxis()->SetLimits(0.,2.);
		if(normalized) haxis->GetYaxis()->SetRangeUser(0.,2.5);
		else if(n>=3) haxis->GetYaxis()->SetRangeUser(0.,0.05);
		else  haxis->GetYaxis()->SetRangeUser(0.,0.22);
// forFrancesca
		haxis->GetXaxis()->SetTitle("#mu_{std} (m^{-1})");
		haxis->GetYaxis()->SetTitle("#frac{VPT}{PN} _{max} - #frac{VPT}{PN} _{min}");
...
*/		haxis->Draw();
		...
		col->Update();
/*//forFrancesca*/		if(h_mustd_vs_VPTPNdiff_distancefit[n][nn]->GetEntries()>3) Util::PrintNoEPS(col, outputfile, outputdir, false);
/*//forFrancesca*/		if(h_mustd_vs_VPTPNdiff_distancefit[n][nn]->GetEntries()>3) Util::PrintEPS(col, outputfile, outputdir);
		col->Clear();
		...
		col->Update();
/*//forFrancesca*/		if(h_mustd_vs_VPTPNdiff_scaled_distancefit[n][nn]->GetEntries()>3) Util::PrintNoEPS(col, outputfile, outputdir, false);
/*//forFrancesca*/		if(h_mustd_vs_VPTPNdiff_scaled_distancefit[n][nn]->GetEntries()>3) Util::PrintEPS(col, outputfile, outputdir);
		col->Clear();
	}
	}
 //forFrancesca
	...
	TFile *newFile;
//	if(normalized) newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_savecopy_TESTEWRIAC_withTDR_withfits_AbsEta.root", "RECREATE");// mustd <-> muSIC
//	if(normalized) newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_savecopy_TESTEWRIAC_withSasha_unscaled_withfits_AbsEta.root", "RECREATE");// mustd <-> muSIC
	if(normalized) newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_muSIC_vs_VPTPNdiff_EWRIAC_withSashaScaled_withfits_AbsEta_v1119.root", "RECREATE");// mustd <-> muSIC
	else newFile = new TFile("/shome/haweber/ECAL/Laser/Analysis/FileOutputs/20120410_mustd_vs_VPTPNmaxMinusVPTPNmin_withfits_AbsEta_nonorm_v1119.root", "RECREATE");
	newFile->cd();
